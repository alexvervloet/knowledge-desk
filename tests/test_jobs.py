...
def test_claim_is_fifo():
    ...
    assert jobs.claim_one()["payload"]["i"] == 1
    assert jobs.claim_one()["payload"]["i"] == 2
...
def test_retry_then_dead_letter():
    ...
    job = jobs.claim_one()
    assert jobs.mark_failed(job["id"], "boom", backoff_seconds=0) == "queued"
    job = jobs.claim_one()
    assert jobs.mark_failed(job["id"], "boom", backoff_seconds=0) == "queued"
    job = jobs.claim_one()
    assert jobs.mark_failed(job["id"], "boom", backoff_seconds=0) == "dead"
