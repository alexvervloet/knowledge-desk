...
| Orchestration | [assistant.py:50-175](../../knowledge_desk/assistant.py#L50-L175 "answer_stream") `answer_stream` | The whole request lifecycle in one readable function. Start here. |
...
| SSE frames over HTTP | [main.py:332-354](../../knowledge_desk/main.py#L332-L354 "ask") `ask` | Event dicts become `data:` lines. Frontend parser: [api.ts](../../frontend/src/api.ts). |
...
| **ACL inside the candidate fetch** | [tenancy.py:383-404](../../knowledge_desk/tenancy.py#L383-L404 "TenantScope.search") `TenantScope.search` | The single most important query in the repo. Filter and ranking in the same SQL. |
| The caller's access set | [tenancy.py:349-363](../../knowledge_desk/tenancy.py#L349-L363 "TenantScope.principals") `principals` | Recomputed per query, never cached, so a group change takes effect immediately. |
...
| 2. ACL in the ranking query | [tenancy.py:383-404](../../knowledge_desk/tenancy.py#L383-L404 "TenantScope.search") | Forbidden rows are never scored. |
...
| Concept | Where | What to notice |
|---|---|---|
| Sync, not append | [ingest.py:34-133](../../knowledge_desk/ingest.py#L34-L133 "sync_documents") `sync_documents` | The upload is the complete desired state. Absent paths get deleted. |
...
| Queue claim, `skip locked` | [jobs.py:48-61](../../knowledge_desk/jobs.py#L48-L61 "claim_one") `claim_one` | Concurrent workers step over each other's locked rows. A queue in Postgres, no broker. |
...
| Concept | Where | What to notice |
|---|---|---|
| One trace per question | [tracing.py:73-164](../../knowledge_desk/tracing.py#L73-L164 "AskTracer") `AskTracer` | Retrieval span + generation with usage and cost, tagged by org and user. |
