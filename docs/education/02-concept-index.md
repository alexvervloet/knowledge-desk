...
|---|---|---|
| System prompt | [providers.py:30-49](../../knowledge_desk/providers.py#L30-L49) `_SYSTEM` | Two jobs in one prompt: answer only from context, and treat context as data. Both are load-bearing. |
...
| Defusing the prompt's grammar | [providers.py:77-154](../../knowledge_desk/providers.py#L77-L154) `_GRAMMAR` | Markers, `[n]` citation keys, and the `path:` line. A passage that writes your grammar gets to lie about what kind of text it is. |
...
| Rendering retrieved passages | [providers.py:162-182](../../knowledge_desk/providers.py#L162-L182) `_render_context` | Everything the uploader supplied goes inside the fence; only the minted `[n]` label stays out. |
...
| Token cost estimate | [providers.py:157-159](../../knowledge_desk/providers.py#L157-L159) `_cost` | Per-million-token pricing table; the only place money is computed. |
| Orchestration | [assistant.py:50-172](../../knowledge_desk/assistant.py#L50-L172) `answer_stream` | The whole request lifecycle in one readable function. Start here. |
| Refusal instead of guessing | [assistant.py:29-32](../../knowledge_desk/assistant.py#L29-L32), [:90-96](../../knowledge_desk/assistant.py#L89-L103) | Empty permitted retrieval → refuse. The access boundary reaching the generated text. |
| SSE frames over HTTP | [main.py:332-354](../../knowledge_desk/main.py#L332-L354) `ask` | Event dicts become `data:` lines. Frontend parser: [api.ts](../../frontend/src/api.ts). |
...
| **ACL inside the candidate fetch** | [tenancy.py:383-404](../../knowledge_desk/tenancy.py#L383-L404) `TenantScope.search` | The single most important query in the repo. Filter and ranking in the same SQL. |
| The caller's access set | [tenancy.py:349-364](../../knowledge_desk/tenancy.py#L349-L364) `principals` | Recomputed per query, never cached, so a group change takes effect immediately. |
...
| 2. ACL in the ranking query | [tenancy.py:383-404](../../knowledge_desk/tenancy.py#L383-L404) | Forbidden rows are never scored. |
...
| Storage quota inside the write txn | [tenancy.py:223-251](../../knowledge_desk/tenancy.py#L223-L251) | Checked in the same transaction that writes, so concurrent uploads cannot race past it. |
...
| Concept | Where | What to notice |
|---|---|---|
| One trace per question | [tracing.py:73-164](../../knowledge_desk/tracing.py#L73-L164) `AskTracer` | Retrieval span + generation with usage and cost, tagged by org and user. |
